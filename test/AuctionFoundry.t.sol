...
import {JunNFT} from "../contracts/JunNFT.sol";
import {AuctionTestFeed, FeeOnTransferTestToken, AuctionReentrantRecipient, AuctionReentrantToken} from "./AuctionFoundry.t.sol";
...
contract AuctionTest is Test, ERC721Holder {
    ...
    function setUp() public {
        ...
        nft = JunNFT(
            address(
                new ERC1967Proxy(
                    address(new JunNFT()),
                    abi.encodeCall(JunNFT.initialize, (100, address(this)))
                )
            )
        );
    ...
    function test_RejectsFeeOnTransferDepositsAndRollsBack() public {
        ...
        vm.prank(bidder);
        // 授权 Auction 合约可以从该地址转走对应 ERC20 / NFT。
        feeToken.approve(address(auction), type(uint256).max);
        feeToken.setFee(1);
        // 下一次外部调用模拟由 bidder 发起，也就是下一次调用里的 msg.sender = bidder。
        vm.prank(bidder);
        // 预期“紧接着的下一次调用”必须回滚；如果下一次调用成功，测试反而会失败。
    ...
    function test_InvalidConfigUpdateLeavesPreviousConfigIntact() public {
        ...
        (address configuredFeed, , , bool enabled, uint256 maxAge) = auction.priceConfigs(address(0));
        // 断言：这里读取到的实际值必须等于期望值，否则说明状态变化不符合设计。
    ...
    function test_UUPSInitializationAndUpgradePreserveState() public {
        ...
        // 预期“紧接着的下一次调用”必须回滚；如果下一次调用成功，测试反而会失败。
        vm.expectRevert(
            abi.encodeWithSignature("ERC1967InvalidImplementation(address)", address(feed))
        );
        // UUPS 升级入口：切换实现合约，并可在同一交易中执行 V2 初始化调用。
    ...
    function test_FuzzLiabilitiesConservedAcrossRefundAndSettlement(
        uint96 first,
        uint96 extra
    ) public {
